...
export default function HeroPage({ productid,title, description, img , price }: HeroPageProps) {
  return (
    <div>
        ...
        <div className="flex h-full max-h-[40rem] w-full max-w-[72remrem] overflow-hidden bg-card rounded-2xl shadow-md">
          <div className="w-full overflow-y-auto p-10 space-y-10 ">
            <div className="h-full flex flex-col items-start justify-center space-y-4   ">
              <h1 className="font-bold text-2xl">{title} <PriceTag price={price} /></h1>
           ...
           <Link href={`/products/${productid}`} className=" bg-yellow-400 px-6 py-2 rounded-2xl font-semibold">
            Check it Out
            </Link>
           </div>
          </div>
